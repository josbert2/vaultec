...
import { EyeIcon, EyeOffIcon, ArrowUpRight } from "lucide-react";
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Button
          type="submit"
          className="h-12 w-full rounded-none bg-white text-base font-semibold text-black hover:bg-zinc-200"
          disabled={isPending}
        >
          {isPending ? "Signing in..." : "SIGN IN"}
          {!isPending && <ArrowUpRight className="ml-2 h-5 w-5" />}
        </Button>
      </form>
    </Form>
  );
